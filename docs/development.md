...
| `test/renderer/components.test.tsx` | React 组件（@testing-library/react）：ChatArea 气泡流式渲染 + 光标 + 工具卡 + ask_user + 发送禁用；LeftSidebar 面板切换 + 列表渲染 + 模式高亮；RightSidebar 指标；TitleBar 窗口控制 IPC |
